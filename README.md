...
O Alinhados é um ecossistema de repositórios independentes e versionados, com o **contrato de dados como fonte única de verdade**.
 
```mermaid
graph TD
    contracts["📦 alinhados-contracts<br/>Schemas Zod + OpenAPI"]
    api["🖥️ alinhados-api<br/>Express · Prisma · PostgreSQL"]
    web["🌐 alinhados-web<br/>Next.js · PWA"]
 
    api -->|Importa DTOs e valida payloads| contracts
    web -->|Compartilha tipos e consome a API| contracts
    web -->|REST / HTTPS| api
```
...
Validados localmente via **Lefthook** — commits e branches fora do padrão são barrados antes do push.
 
### Branches
- `main` (produção) · `develop` (staging)
- `feature/<US-ID>-descricao` · `fix/<épico>-descricao` · `hotfix/<bug-ID>-descricao` · `release/vX.Y.Z`
### Commits (Conventional Commits)
`<tipo>(<escopo opcional>): <descrição no imperativo>` — tipos permitidos: `feat`, `fix`, `refactor`, `ci`, `docs`, `test`, `chore`.
