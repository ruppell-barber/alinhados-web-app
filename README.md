...
O Alinhados é um ecossistema de repositórios independentes e versionados, com o contrato de dados como fonte única de verdade. A arquitetura é descrita no modelo C4.

Nível 1, Contexto
plantuml
@startuml c4_contexto
!include <C4/C4_Context>
title Alinhados, Diagrama de Contexto (C4 Nivel 1)
Person(barbeiro, "Barbeiro", "Busca recolocacao ou melhor comissao")
Person(barbearia, "Barbearia", "Busca preencher cadeiras")
Person(admin, "Administrador", "Modera o marketplace")
System(alinhados, "Alinhados", "Match entre barbeiros e barbearias (PWA)")
System_Ext(supabase, "Supabase", "Auth, PostgreSQL e Storage")
Rel(barbeiro, alinhados, "Cria perfil, descobre, conversa", "HTTPS")
Rel(barbearia, alinhados, "Publica vagas, descobre, conversa", "HTTPS")
Rel(admin, alinhados, "Modera e mede", "HTTPS")
Rel(alinhados, supabase, "Persiste dados, arquivos e sessao", "HTTPS")
@enduml

🔗 Ver diagrama renderizado

Nível 2, Contêineres
plantuml
@startuml c4_container
!include <C4/C4_Container>
title Alinhados, Diagrama de Conteineres (C4 Nivel 2)
Person(usuario, "Barbeiro / Barbearia / Admin")
System_Boundary(alinhados, "Alinhados") {
  Container(web, "alinhados-web", "Next.js, PWA", "Interface de barbeiro, barbearia e admin")
  Container(api, "alinhados-api", "Node.js, Express, Prisma", "Monolito modular hexagonal com as regras de negocio")
  Container(contracts, "alinhados-contracts", "TypeScript, Zod", "Contrato de dados compartilhado")
  ContainerDb(db, "Banco de Dados", "PostgreSQL (Supabase)", "Perfis, swipes, matches, mensagens")
}
System_Ext(supabase, "Supabase Auth / Storage", "Autenticacao e imagens")
Rel(usuario, web, "Usa", "HTTPS")
Rel(web, api, "Consome", "REST / HTTPS")
Rel(web, contracts, "Compartilha tipos", "import")
Rel(api, contracts, "Valida payloads", "import")
Rel(api, db, "Le e escreve", "SQL")
Rel(api, supabase, "Auth e upload de imagens", "HTTPS")
@enduml
...
Staging, push em develop: lint, testes e build de preparação.
Homologação, branches release/*: verificação completa de integração para o aceite (UAT).
Produção, merge em main: build, testes E2E e deploy (Vercel), com aprovação manual antes de subir.
Promoção stable, tags v*: publica a imagem no registry com o sufixo stable.
